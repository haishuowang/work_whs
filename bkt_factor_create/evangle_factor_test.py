...
class FactorTest(FactorTestBase, DiscreteClass, ContinueClass, TrainFunSet):
    ...
    def get_mix_pnl_df(self, exe_str, cut_date, percent):
        def tmp_fun():
            ...
            for i in range(int((len(exe_list) - 1) / 2)):
                ...
                factor_2 = self.load_raw_factor(name_2) * float(way_str_2)
    ...
    def train_fun(self, cut_date, percent):
        try:
            ...
            select_factor = random.sample(all_file, 50)
            evangle_list = [
                'PEG_EBIT_3Y',
                'PEG_EBIT_5Y',
                'PEG_OPCF_3Y',
            ...
            add_factor = [x for x in all_file if x.split('|')[0] in evangle_list]
            # 把想加入的factor加进去
            select_factor = list(set(select_factor + add_factor))
            # select_factor = add_factor
            ...
            for low_corr_factor in low_corr_factor_scores.index:
                ...
                if tmp_score > mix_score:
                    ...
                    use_factor_list.append(low_corr_factor)
                    ...
                    if mix_num >= 10:
                        break
            if mix_info.loc['sp'] > 2 and mix_info.loc['pot'] > 50 and \
                    len(set(use_factor_list) & set(add_factor)) > 0:

                result_save_path = f'/mnt/mfs/dat_whs/result_new/evangle/'
    ...
    def run(self, cut_date, percent, run_num=1):
        pool = Pool(28)
        args = (cut_date, percent)

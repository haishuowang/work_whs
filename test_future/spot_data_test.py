...
#         self.act_fut_sr = self.load_act_fut_data_r(fut_name, 'adj_r', active_df)
#
...
#
#     def spot_test(self, fut_name, spot_name, fun_name, lag=1, way=1):
#         print(spot_name)
#         # load spot data
#         # all_spot_data = self.load_spot_data(fut_name, spot_name)
#         print(fut_name, spot_name)
...
#         # 数据回测
#
#         pos_df = getattr(self, fun_name)(all_spot_data).reindex(index=self.act_fut_sr.index)
#         pnl_df = pos_df.shift(lag).mul(self.return_df, axis=0)
#         # 指标计算
#
#         ################################
#         # all_spot_data_mean = bt.AZ_Rolling_mean(all_spot_data, 1)
#         # # tmp_df = all_spot_data_mean / all_spot_data_mean.shift(1) - 1
#         # tmp_df = all_spot_data / all_spot_data.shift(1) - 1
#         # tmp_df = tmp_df.replace(0, np.nan).fillna(method='ffill')
#         #
#         # tmp_df[tmp_df.abs() > 0.05] = 0.05
#         # tmp_df[tmp_df.abs() < -0.05] = -0.05
#         # # tmp_df[tmp_df.abs() > 0.02] =
#         #
#         # # pos_df = tmp_df.applymap(lambda x: 0 if (abs(x) > 0.02) & (abs(x) < 0.01) else(1 if x > 0 else -1))
#         # pos_df = tmp_df.applymap(lambda x: 0 if abs(x) > 0.04 else(1 if x > 0 else -1))
#         # pnl_df = pos_df.shift(lag).mul(self.return_df, axis=0)
#
#         asset_df = pnl_df.sum()
...
#         #     SignalAnalysis.CDF(part_tmp_df, self.return_df, 1, title=f'{col} CDF Figure', lag=lag)
#         return pos_df * way, pnl_df[select_data].sum(1) * way
#
#     def all_spot_test(self, select_spot_name_dict, lag=1):
...
#     spot_data_test = SpotDataTest(begin_time, end_time, root_path, fut_name, hold_time, lag=lag, use_num=1)
...
end_time = pd.to_datetime('2018-01-01')

fut_data = FutData()
root_path = '/mnt/mfs/DAT_PUBLIC/spot_data'
path_name_list = sorted(os.listdir(root_path))
path_name = path_name_list[0]
file_name_list = sorted(os.listdir(f'{root_path}/{path_name}'))
...
plot_send_data(pnl_df.cumsum(), subject='test')
a = bt.AZ_Sharpe_y(pnl_df).sort_values(na_position='last')

# 鸡棚前价格:817小规格:河北:衡水    13.623753
# 鸡棚前价格:817小规格:河北:沧州    13.479122
# 价格:肉鸡苗:河南:开封          19.812484
# 鸡棚前价格:817小规格:河北:廊坊    15.739966

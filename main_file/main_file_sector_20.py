...
class FactorTestSector(mf.FactorTest):
    ...
    def part_test_index_3_(self, key, name_1, name_2, name_3, log_save_file, result_save_file, total_para_num):
        ...
        factor_1 = self.load_remy_factor(name_1, sector_name)
        factor_2 = self.load_remy_factor(name_2, sector_name)
        factor_3 = self.load_remy_factor(name_3, sector_name)
    ...
    def single_test(self, fun_name, name1, name2, name3):
        ...
        factor_1 = self.load_remy_factor(name1, self.sector_name)
        factor_2 = self.load_remy_factor(name2, self.sector_name)
        factor_3 = self.load_remy_factor(name3, self.sector_name)
        mix_factor = fun(factor_1, factor_2, factor_3)
        daily_pos = self.deal_mix_factor(mix_factor).shift(2)
...
def main_fun(sector_name, hold_time, if_only_long, time_para_dict):
    ...
    part_df = data[(data['hold_time'] == str(hold_time)) & (data['if_only_long'] == str(if_only_long))]

    pnl_table = get_pnl_table(part_df, sector_name)

    sharpe_df = pnl_table.apply(bt.AZ_Sharpe_y)
    sharpe_df = sharpe_df[sharpe_df.abs() > 0.8]
    sharpe_df = sharpe_df.sort_values()
    print('---------------------------------')
    print(len(sharpe_df.index))
    if len(sharpe_df.index) < 4:
        return -1
    corr_df = pnl_table[sharpe_df.index].corr()
    corr_df = corr_df.reindex(index=sharpe_df.index, columns=sharpe_df.index)
    use_file_set = corr_matrix(corr_df)
    use_factor_set = [x.split('|')[0] for x in use_file_set]

    # print(corr_df.reindex(index=use_file_set, columns=use_file_set))
    print(len(use_factor_set))
    # print(use_factor_set)
    # sharpe_df
    main.test_index_3_(use_factor_set, use_factor_set, use_factor_set, pool_num, suffix_name=suffix_name)

...
class FactorTestSector(mf.FactorTest):
    ...
    def part_test_index_3_(self, key, name_1, name_2, name_3, log_save_file, result_save_file, total_para_num):
        ...
        change_factor = self.load_remy_factor(name_1)
        ratio_factor = self.load_remy_factor(name_2)
        tech_factor = self.load_remy_factor(name_3)
    ...
    def single_test(self, fun_name, name1, name2, name3):
        ...
        change_factor = self.load_remy_factor(name1)
        ratio_factor = self.load_remy_factor(name2)
        tech_factor = self.load_remy_factor(name3)
        mix_factor = fun(change_factor, ratio_factor, tech_factor)
        daily_pos = self.deal_mix_factor(mix_factor).shift(2)
...
def main_fun(sector_name, hold_time, if_only_long, time_para_dict):
    ...
    remyMFF_list = [x for x in os.listdir('/mnt/mfs/DAT_EQT/EM_Funda/DERIVED_F1') if 'MFF' in x]
    remyMTK_list = [x for x in os.listdir('/mnt/mfs/DAT_EQT/EM_Funda/DERIVED_F1') if 'MTK' in x]
    remyMWB_list = [x for x in os.listdir('/mnt/mfs/DAT_EQT/EM_Funda/DERIVED_F1') if 'MWB' in x]

    pool_num = 20
    suffix_name = os.path.basename(__file__).split('.')[0].split('_')[-1]
    main.test_index_3_(remyMFF_list, remyMTK_list, remyMWB_list, pool_num, suffix_name=suffix_name)

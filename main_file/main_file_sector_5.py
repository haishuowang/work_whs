...
class FactorTestSector(mf.FactorTest):
    ...
    def load_yolanda_factor(self, file_name):
        load_path = '/mnt/mfs/DAT_PUBLIC/yolanda'
        raw_df = pd.read_pickle(os.path.join(load_path, file_name)) \
            .reindex(index=self.xinx, columns=self.xnms)
        raw_df.index = pd.to_datetime(raw_df.index.astype(str))
        target_df = self.row_extre(raw_df, self.sector_df, 0.3)
        return target_df
    ...
    def part_test_index_3_(self, key, name_1, name_2, name_3, log_save_file, result_save_file, total_para_num):
        ...
        change_factor = self.load_yolanda_factor(name_1)
    ...
    def single_test(self, fun_name, name1, name2, name3):
        ...
        change_factor = self.load_yolanda_factor(name1)
...
if __name__ == '__main__':
    ...
    for sector_name in sector_name_list:
        ...
        vsMCap_list = ['DII_INFO_AN_NEWS_7h.pk']
        ...
        pool_num = 20
        main.test_index_3_(vsMCap_list, ratio_list, tech_list, pool_num,  suffix_name='5')

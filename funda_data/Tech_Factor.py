    ...
    def BBANDS(Close, timeperiod=5, nbdevup=2, nbdevdn=2, matype=0):
        ...
        for i in Close.columns:
            if Close[i].isnull().prod() != 1:
                ...
                upperband[i], middleband[i], lowerband[i] = [np.nan] * len(Close[i]), \
                                                            [np.nan] * len(Close[i]), \
                                                            [np.nan] * len(Close[i])
...
class TechFactorAlpha(FactorAlpha, BaseDeal):
    def __init__(self, root_path, sector_df, save_root_path):
        ...
        if sector_name.startswith('market_top_300plus'):
            self.index_weight = [1, 0]
        elif sector_name.startswith('market_top_300to800plus'):
            self.index_weight = [0, 1]
        else:
            self.index_weight = [0.5, 0.5]
        self.all_index_df_path = root_path.EM_Funda.INDEX_TD_DAILYSYS / 'CHG.csv'
        self.all_index_df = bt.AZ_Load_csv(self.all_index_df_path)
        self.save_root_path = save_root_path

    def MACD_alpha_(self, fastperiod, slowperiod, signalperiod):
        target_df = self.MACD_alpha(self.aadj_r, self.all_index_df, self.sector_df,
                                    fastperiod, slowperiod, signalperiod, self.index_weight)
        iw_str = '_'.join([str(x) for x in self.index_weight])
        file_name = f'MACD_alpha_{fastperiod}_{slowperiod}_{signalperiod}_{iw_str}'
        fun = 'Tech_Factor.FactorAlpha.MACD_alpha'
        raw_data_path = (self.aadj_r_path, self.all_index_df_path)
        args = (fastperiod, slowperiod, signalperiod, self.index_weight)
        self.judge_save_fun(target_df, file_name, self.save_root_path, fun, raw_data_path, args)

    def MA_LINE_alpha_(self, long_short_list):
        for slowperiod, fastperiod in long_short_list:
            target_df = self.MA_LINE_alpha(self.aadj_r, self.all_index_df, self.sector_df,
                                           slowperiod, fastperiod, self.index_weight)
            iw_str = '_'.join([str(x) for x in self.index_weight])
            file_name = f'MA_LINE_alpha_{fastperiod}_{slowperiod}_{iw_str}'
            fun = 'Tech_Factor.FactorAlpha.MA_LINE_alpha'
            raw_data_path = (self.aadj_r_path, self.all_index_df_path)
            args = (fastperiod, slowperiod, self.index_weight)
            self.judge_save_fun(target_df, file_name, self.save_root_path, fun, raw_data_path, args)

    def BBANDS_alpha_(self, n_list, limit_list):
        for n in n_list:
            for limit_up_dn in limit_list:
                target_df = self.BBANDS_alpha(self.aadj_r, self.all_index_df, self.sector_df,
                                              n, limit_up_dn, self.index_weight)
                iw_str = '_'.join([str(x) for x in self.index_weight])
                file_name = f'BBANDS_alpha_{n}_{limit_up_dn}_{iw_str}'
                fun = 'Tech_Factor.FactorAlpha.BBANDS_alpha'
                raw_data_path = (self.aadj_r_path, self.all_index_df_path)
                args = (n, limit_up_dn, self.index_weight)
                self.judge_save_fun(target_df, file_name, self.save_root_path, fun, raw_data_path, args)

    def pnd_continue_ud_alpha_(self, n_list=[3, 4, 5]):
        target_df = self.pnd_continue_ud_alpha(self.aadj_r, self.all_index_df, self.sector_df,
                                               n_list, self.index_weight)
        n_list_str = ''.join([str(x) for x in n_list])
        iw_str = '_'.join([str(x) for x in self.index_weight])
        file_name = f'pnd_continue_ud_alpha{n_list_str}_{iw_str}'
        fun = 'Tech_Factor.FactorAlpha.pnd_continue_ud_alpha'
        raw_data_path = (self.aadj_r_path, self.all_index_df_path)
        args = (n_list, self.index_weight)
        self.judge_save_fun(target_df, file_name, self.save_root_path, fun, raw_data_path, args)

    def pnd_continue_pct_ud_alpha_(self, n_list=[3, 4, 5]):
        target_df = self.pnd_continue_pct_ud_alpha(self.aadj_r, self.all_index_df, self.sector_df,
                                                   n_list, self.index_weight)
        n_list_str = ''.join([str(x) for x in n_list])
        iw_str = '_'.join([str(x) for x in self.index_weight])
        file_name = f'pnd_continue_pct_ud_alpha{n_list_str}_{iw_str}'
        fun = 'Tech_Factor.FactorAlpha.pnd_continue_pct_ud_alpha'
        raw_data_path = (self.aadj_r_path, self.all_index_df_path)
        args = (n_list, self.index_weight)
        self.judge_save_fun(target_df, file_name, self.save_root_path, fun, raw_data_path, args)

    def RSI_alpha_(self, n_list, limit_list):
        for n in n_list:
            for limit_up_dn in limit_list:
                target_df = self.RSI_alpha(self.aadj_r, self.all_index_df, self.sector_df,
                                           n, limit_up_dn, self.index_weight)
                iw_str = '_'.join([str(x) for x in self.index_weight])
                file_name = f'RSI_alpha{n}_{limit_up_dn}_{iw_str}'
    ...
    def WILLR_(self, n_list, limit_up_dn_list):
        for n in n_list:
            for limit_up_dn in limit_up_dn_list:
                ...
                raw_data_path = (self.aadj_p_HIGH_path, self.aadj_p_LOW_path, self.aadj_p_HIGH_path)
                args = (n, limit_up_dn)
                self.judge_save_fun(target_df, file_name, self.save_root_path, fun, raw_data_path, args)
...
def main_alpha(sector_df, root_path, save_root_path):
    limit_list = [1, 1.5, 2]
    n_list = [10, 20, 40, 100, 140, 200]
    long_short_list = [(5, 10), (20, 60), (40, 100), (60, 120), (60, 160)]
    techfactoralpha = TechFactorAlpha(root_path, sector_df, save_root_path)
    techfactoralpha.MACD_alpha_(fastperiod=12, slowperiod=26, signalperiod=9)
    techfactoralpha.MACD_alpha_(fastperiod=20, slowperiod=60, signalperiod=18)
    techfactoralpha.MA_LINE_alpha_(long_short_list)
    techfactoralpha.BBANDS_alpha_(n_list, limit_list)
    techfactoralpha.pnd_continue_ud_alpha_([3, 4, 5])
    techfactoralpha.pnd_continue_pct_ud_alpha_([3, 4, 5])
    techfactoralpha.RSI_alpha_(n_list, limit_list)

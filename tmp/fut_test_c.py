...
def date_fun(date_now, fut_name):
    ...
    for con_file in con_list:
        ...
        con_data['Volume'] = con_data['Trade_num'].diff()
        # except:
        #     print(1)
        ...
        bid_ask_data = con_data[['Now_px', 'Volume', 'bid_px', 'bid_num', 'ask_px', 'ask_num']]
...
def pnl_fun(date_now, fut_name='rb', bkt_num=3600, buy_sell_list=[0.05, 0.90, 0.95, 0.10]):
    ...
    x = pd.concat([diff_df[[next_name] + [i + j for i in [con_name, next_name] for j in col_list]]
                  .iloc[bkt_num:], pos_sr], axis=1)

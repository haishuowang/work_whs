...
def filter_ic_sharpe(cut_date, signal, pct_n, lag=1):
    ...
    cut_sharpe = bt.AZ_Rolling_sharpe(pnl_df_in, roll_year=1, year_len=250, min_periods=1,
                                      cut_point_list=[0.3, 0.7], output=False)

    ic_in_condition_1 = ic_rolling_5_y_mean > 0.04 and cut_sharpe.values[0] > 1.5
    ic_in_condition_2 = ic_rolling_5_y_mean < -0.04 and cut_sharpe.values[1] < -1.5
    ...
    return ic_in_condition, out_condition, ic_rolling_5_y_mean, ic_rolling_half_y_quantile, sharpe_quantile, leve_ratio
...
def part_test_index_3_smart(key, name_1, name_2, name_3, begin_date, cut_date, stock_universe, return_choose,
    ...
    filer_name = filter_ic_sharpe.__name__
    for fun in fun_mix_2_set:
        mix_factor = fun(factor_set[name_1], factor_set[name_2], factor_set[name_3])
        in_condition, *filter_result = filter_ic_sharpe(cut_date, mix_factor, return_choose, lag=1)
        if in_condition:
            if if_save:
                with lock:

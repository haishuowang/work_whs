...
class FactorTestSector(mf.FactorTest):
    ...
    def part_test_index_3_(self, key, name_1, name_2, name_3, log_save_file, result_save_file, total_para_num):
        ...
        change_factor = self.load_vsMCap_factor(name_1)
        ...
        for fun in fun_mix_2_set:
            ...
            if len(mix_factor.abs().sum(axis=1).replace(0, np.nan).dropna()) / len(mix_factor) < 0.50:
                continue
...
if __name__ == '__main__':
    ...
    for sector_name in sector_name_list:
        ...
        pool_num = 10
        main.test_index_3_(vsMCap_list, ratio_list, tech_list, pool_num, suffix_name='2')

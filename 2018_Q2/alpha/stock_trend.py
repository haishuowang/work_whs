...
def signal_MACD(price, n_fast, n_slow, n_diff=9):
    EMAfast = pd.Series(pd.ewma(price, span=n_fast, min_periods=n_slow - 1))
    EMAslow = pd.Series(pd.ewma(price, span=n_slow, min_periods=n_slow - 1))
    MACD = pd.Series(EMAfast - EMAslow, name='MACD_' + str(n_fast) + '_' + str(n_slow))
    MACDsign = pd.Series(pd.ewma(MACD, span=n_diff, min_periods=n_diff-1), name='MACDsign_' + str(n_fast) + '_' + str(n_slow))
...
def daily_trade_fun(i_date, date_list, vol_num, log_time, n, limit, stock_num):
...
def back_test_fun(vol_num, log_time, n, limit, stock_num):
    ...
    for i_date in range(vol_num, len(date_list)):
        ...
        pnl_list[i_date], trade_list[i_date] = daily_trade_fun \
            (i_date, date_list, vol_num, log_time, n, limit, stock_num)
...
if __name__ == '__main__':
    ...
    # 选股数量
    stock_num = 200
    global price, volume, vwap, log_save_path
    ...
    log_save_path = "/home/whs/Work/result/bbangs/log_bbangs_pos1.txt"
    ...
    if os.path.exists(log_save_path):
        log_df = pd.read_csv('/home/whs/Work/result/bbangs/log.txt', sep=',', header=None, index_col=0)
    else:
        log_df = float('nan')

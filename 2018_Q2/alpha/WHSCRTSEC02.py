...
def get_mix_factor(config_data, main):
...
def config_test():
    config_set = pd.read_pickle(f'/mnt/mfs/alpha_whs/CRTSECJUN04.pkl')
    config_data = config_set['factor_info']

    filter_cond = config_data[['name1', 'name2', 'name3']] \
        .apply(lambda x: not ('turn_p150d_0.18' in set(x)), axis=1)
    config_data = config_data[filter_cond]

...
def split_file_fun(limit=1000, file_size=40):
    ...
    file_list = sorted([x[:-4] for x in os.listdir(load_path)])
...
def load_split_index(begin_date, end_date, key_set, split_file_set):
    ...
    for key in key_set:
        ...
        for file_name in split_file_set[key]:
            ...
            sub_index_set[file_name] = target_df[(target_df.index <= end_date) & (target_df.index >= begin_date)]
            del target_df
...
def create_all_para(split_file_set, key_comb):
    load_path = r'/media/hdd0/whs/data/adj_data/index_universe'
    file_list = sorted(os.listdir(load_path))
    file_name_list = [x[:-4] for x in file_list]
    file_name_comb = list(combinations(file_name_list, 3))

    return_load_path = '/media/hdd0/whs/data/adj_data/fnd_pct'
    return_list = sorted(os.listdir(return_load_path))
    return_name_list = [x[:-4] for x in return_list]

    fun_set = [mul_fun, sub_fun, add_fun]
    fun_mix_2_set = create_fun_set_2(fun_set)
    key_comb_file_name_set = {}
    for key_set in key_comb:
        key_comb_file_name_set[key_set] = reduce(lambda x, y: x+y, map(lambda x: split_file_set[x], key_set))

    for index_name_1, index_name_2, index_name_3 in file_name_comb:
            for return_name in return_name_list:
                for key_set in key_comb:
                    key_set_file_name = key_comb_file_name_set[key_set]
                    if {index_name_1, index_name_2, index_name_3}.issubset(key_set_file_name):
                        yield key_set, index_name_1, index_name_2, index_name_3, return_name
                        break


def create_para_generator(fun, index_set, return_set):
    if len(index_set) == 1:
        ...
        if len(index_set_1) >= 3:
            ...
            for (index_name_1, index_name_2, index_name_3), f_return_name in index_return_comb:
                ...
                yield (index_name_1, index_name_2, index_name_3, f_return_name), \
                      (index_choose_1, index_choose_2, index_choose_3, f_return_choose)
        else:
            pass
...
def part_test_index_3_smart(i, begin_date, end_date, key_set, split_file_set, return_set, save_log_file):
    lock = Lock()
    ...
    for name_list, (index_choose_1, index_choose_2, index_choose_3, f_return_choose) \
            in create_para_generator(fun, index_set, return_set):
        start_time = datetime.now()

        (index_name_1, index_name_2, index_name_3, f_return_name) = name_list
        ...
        if condition:
            with lock:
                ...
                cost_time = (end_time - start_time).seconds + (end_time - start_time).microseconds * 10 ** -6
                write_str = '|'.join([str(key_set), fun.__name__, index_name_1, index_name_2, index_name_3,
                                      f_return_name, str(ic_mean), '|'.join([str(round(x, 4)) for x in cut_sharpe]),
                                      str(round(leve_ratio, 4)), str(round(cost_time, 4)), ]) + '\n'
                f.write(write_str)
                f.close()
            print(write_str)
        del index_choose_1, index_choose_2, index_choose_3
        gc.collect()
    else:
        pass
...
def test_index_3_smart(begin_date, end_date):
    ...
    split_file_set = OrderedDict(zip(range(len(split_file_list)), split_file_list))
    ...
    save_log_file = '/home/whs/Work/result/factor_search/tmp_{}.txt'.format(datetime.now().strftime("%Y%m%d_%H%M"))
    load_log_file = os.path.join('/home/whs/Work/result/factor_search/', 'tmp.txt')
    log_df= pd.read_table(load_log_file, header=None, sep='|')

    if not os.path.exists(save_log_file):
        os.mknod(save_log_file)
    pool = Pool(6)
    for key_set in key_comb:
        fun_set = [mul_fun, dec_fun, add_fun]
        fun_mix_2_set = create_fun_set_2(fun_set)
        for i in range(len(fun_mix_2_set)):
            pool.apply_async(part_test_index_3_smart,
                             args=(i, begin_date, end_date, key_set, split_file_set, return_set, save_log_file))

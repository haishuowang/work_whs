...
def get_data(target_path, date):
    part_df = pd.read_pickle(target_path)
    part_df.columns = [date]
    return part_df.T


def get_group_info():
    root_path = '/mnt/mfs/dat_whs/tmp/kmean_intra_res'
    ...
    for file_name in file_name_list:
        print(file_name)
        date = file_name[:-4]
        args = (f'{root_path}/{file_name}', date,)
        get_data(*args)
        # group_info_df = pd.concat([group_info_df, part_df], axis=1, sort=False)
        result_list.append(pool.apply_async(get_data, args))
    pool.close()
    pool.join()
    group_info_df = pd.concat([res.get() for res in result_list], sort=False)
    return group_info_df


def part_fun(target_path, date, AR1, limit=0):
    try:
        daily_df = pd.read_pickle(target_path)
        daily_df.columns = [date]
        stock_daily_df = pd.DataFrame()
        for i in range(30):
            group_daily_df = daily_df[daily_df == i].dropna()
            ARI = AR1.loc[pd.to_datetime(date)][group_daily_df.index]
            group_score = ARI.mean()
            if abs(group_score) > limit:
                if group_score > 0:
                    ARI.iloc[:] = 1
                else:
                    ARI.iloc[:] = -1
                stock_daily_df = pd.concat([stock_daily_df, ARI], sort=False)
        print(date)
        stock_daily_df.columns = [date]
        return stock_daily_df.T
    except:
        return pd.DataFrame(columns=[date])


def main_fun(AR1, limit):
    root_path = '/mnt/mfs/dat_whs/tmp/kmean_intra_res'
    file_name_list = sorted(os.listdir(root_path))
    ...
    for file_name in file_name_list:
        date = file_name[:-4]
        args = (f'{root_path}/{file_name}', date, AR1, limit)
        # part_fun(*args)
        result_list.append(pool.apply_async(part_fun, args))
    pool.close()
    pool.join()
    return pd.concat([res.get() for res in result_list], axis=1, sort=False)


def main_fun_1(sector_name, hold_time, if_only_long, time_para_dict, long_short, back_look_day):
    ...
    signal_df = main_fun(AR1, limit=0.1)
...
def tmp_fun():
    root_path = '/mnt/mfs/dat_whs/tmp/kmean_intra_res'
    csv_file_list = [x for x in os.listdir(root_path) if x.endswith('csv')]
    result_list = []
    pool = Pool(25)
    for csv_file in sorted(csv_file_list):
        result_list.append(pool.apply_async(part_tmp, (f'{root_path}/{csv_file}', csv_file[:-4])))
    pool.close()
    pool.join()

    return pd.concat([res.get() for res in result_list], sort=False)


target_df = tmp_fun()
target_df.to_csv('/mnt/mfs/dat_whs/tmp/k_mean_group.csv', sep='|')
...
#         main_fun_1(sector_name, hold_time, if_only_long, time_para_dict, long_short, back_look_day)

...
class KeyFun:
    ...
    @staticmethod
    def load_tech_data(filter_name, xinx, xnms, sector_df, if_only_long):
    ...
    def ic_if_diff(xinx):
        ...
        hs300 = data['000300'].reindex(index=xinx) * 0.01
        zz500 = data['000905'].reindex(index=xinx) * 0.01

        hs300_mean = bt.AZ_Rolling_mean(hs300, 5, 0)
        zz500_mean = bt.AZ_Rolling_mean(zz500, 5, 0)
    ...
    def load_filter_data(self, filter_name, xinx, xnms, sector_df, if_only_long):
        tmp_df = self.ic_if_diff(xinx)
        target_df = pd.DataFrame(np.array([tmp_df.values.ravel()] * len(xnms)).T, index=xinx, columns=xnms)
        if if_only_long:
            target_df = target_df[target_df > 0]
        return target_df

    def create_mix_factor(self, name_1, xinx, xnms, sector_df, if_only_long, percent):
        factor_1 = self.load_tech_data(name_1, xinx, xnms, sector_df, if_only_long)
        # factor_hs_zz = self.ic_if_diff(xinx)
...
def main_fun(time_para_dict, sector_name, hold_time, if_only_long):
    ...
    main_model.main_test_fun(['filter_HS300'], tech_list, pool_num=pool_num, suffix_name=suffix_name, old_file_name='')

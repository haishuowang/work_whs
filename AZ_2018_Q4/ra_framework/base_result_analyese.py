...
# result_data.columns = ['time_para', 'key', 'name1', 'name2', 'name3', 'sector_name', 'con_in', 'con_out_1',
#                        'con_out_2', 'con_out_3', 'con_out_4', 'ic', 'sp_u', 'sp_m', 'sp_d', 'pot_in', 'fit_ratio',
#                        'leve_ratio', 'sp_in', 'sp_out_1', 'sp_out_2', 'sp_out_3', 'sp_out_4']
...
def config_create(main_model, sector_name, result_file_name, config_name, data, time_para, pot_in_num, leve_ratio_num,
    ...
    a_n['pnl_file_name'] = a_n[['time_para', 'key']].apply(lambda x: '|'.join(x.astype(str)), axis=1)
...
def bkt_fun(main_model, pnl_save_path, a_n, i):
    print(1)
    x, key, name1, name2, name3, sector_name, \
    ...
    sp_in_c, sp_out_c, pnl_df_c = main_model.single_test(name1, name2, name3)
    plot_send_result(pnl_df_c, bt.AZ_Sharpe_y(pnl_df_c), '{}, key={}'.format(i, key))

    print('***************************************************')
    print('now {}\'s is running, key={}, {}, {}, {}'.format(i, key, name1, name2, name3))
    ...
    if sp_m > 0:
        if not os.path.exists(os.path.join(pnl_save_path, '{}|{}.csv'.format(x, key))):
            pnl_df_c.to_pickle(os.path.join(pnl_save_path, '{}|{}.csv'.format(x, key)))
...
def pos_sum_c(if_only_long, time_para_dict, main_model, data, time_para, result_file_name,
    ...
    data_n = data[data['time_para'] == time_para]
    begin_date, cut_date, end_date, _, _, _ = time_para_dict[time_para]
    a_n = data_n[(data_n['ic'].abs() >= ic_num) &
                 (data_n['pot_in'].abs() > pot_in_num) &
                 (data_n['leve_ratio'].abs() > leve_ratio_num) &
                 (data_n['sp_in'].abs() > sp_in) &
    ...
    fit_ratio, leve_ratio, sp_in, sharpe_q_out, pnl_df = filter_all(cut_date, sum_pos_df, main_model.return_choose,
                                                                    if_return_pnl=True, if_only_long=if_only_long)
...
def find_target_file(begin_time, end_time, time_type, endswith=None):
    ...
    result_root_path = '/mnt/mfs/dat_whs/result_new/result'
...
def main(result_file_name):
    ...
    loc = locals()
    exec(f'from work_whs.AZ_2018_Q4.bkt_framework import main_file_{script_num} as mf')
    # from work_whs.AZ_2018_Q4.bkt_framework import main_file_1 as mf
    mf = loc['mf']
    ...
    result_path = '/mnt/mfs/dat_whs/result_new/result/{}.txt'.format(result_file_name)
    # #############################################################################
    # 判断文件大小
    print('判断文件大小')
    if os.path.getsize(result_path):
        ...
        # data.columns = ['time_para', 'key', 'name1', 'name2', 'name3', 'sector_name', 'con_in', 'con_out_1',
        #                 'con_out_2', 'con_out_3', 'con_out_4', 'ic', 'sp_u', 'sp_m', 'sp_d',
        #                 'pot_in', 'fit_ratio', 'leve_ratio', 'sp_in', 'sp_out_1', 'sp_out_2', 'sp_out_3',
        #                 'sp_out_4']
        name_list = [f'name{x+1}' for x in range(len(data.columns)-20)]
        data.columns = ['time_para', 'key'] + name_list + \
                       ['sector_name', 'con_in', 'con_out_1', 'con_out_2',
                        'con_out_3', 'con_out_4', 'ic', 'sp_u', 'sp_m',
                        'sp_d', 'pot_in', 'fit_ratio', 'leve_ratio',
                        'sp_in', 'sp_out_1', 'sp_out_2', 'sp_out_3',
                        'sp_out_4']

    else:
        return 0

    filter_cond = data[['name1', 'name2']] \
    ...
    para_adj_set_list = [{'pot_in_num': 0, 'leve_ratio_num': 0, 'sp_in': 0, 'ic_num': 0.0, 'fit_ratio': 0}]
    time_para = 'time_para_6'
    print(time_para)
    ...
    key_fun = mf.KeyFun()
    main_model = mf.bkt_base.FactorTest(key_fun, *para_set)
    sum_pos_df, pnl_df = pos_sum_c(if_only_long, time_para_dict, main_model, data, time_para, result_file_name,
                                   **survive_result)
    #############################################################################
    # 生成config文件
    ...
    print('______________________________________')
    print(data)
    return data[['pot_in_num', 'leve_ratio_num', 'sp_in', 'ic_num', 'fit_ratio']]
...
if __name__ == '__main__':
    ...
    begin_time = datetime(2018, 12, 19, 18, 11, 13)
    end_time = datetime(2018, 12, 23, 18, 30, 13)

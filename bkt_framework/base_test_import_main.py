...
class FactorTest(FactorTestBase):
    ...
    def get_mix_low_factor(self, best_factor, best_score, low_corr_df, select_info_df, cut_date, percent):
        ...
        for low_corr_factor in low_corr_df.index:
            ...
            tmp_score = self.get_score_fun(tmp_info_df['sp_in'], tmp_info_df['pot_in'])
    ...
    def get_scores_fun(sp_in, pot_in):
        return (sp_in.where(sp_in > 0.4, other=0)) + 5 * (sp_in > 2) + 10 * (sp_in > 2.3) + \
               (pot_in.where(pot_in < 100, other=100)) / 200
        # + 1 * (pot_in > 50)

    @staticmethod
    def get_score_fun(sp_in, pot_in):
        tmp_fun = lambda x: x if x > 0.4 else 0
        return tmp_fun(sp_in) + min(pot_in, 100) / 200 + 5 * (sp_in > 2) + 10 * (sp_in > 2.3)
        # + 10 * (pot_in > 50)
    ...
    def train_fun(self, fun_use_num, cut_date, percent, evol_num=6, if_multy=False):
        try:
            result_save_path = '/mnt/mfs/dat_whs/result_new2/test01'
            ...
            while loop_now < loop_max:
                ...
                select_factor_scores = self.get_scores_fun(select_sp_in, select_pot_in) \
                ...
                if select_factor_scores.iloc[0] > best_score:
                    ...
                    low_corr_df = select_pnl_df[select_factor_scores.index[:evol_keep_num]].corr()[str(best_factor)] \
                ...
                for x in delete_factor_list:
                    ...
                    # self.factor_way_dict.pop(str(x))
                # delete_file_list = [self.mix_factor_name_dict[str(x)] for x in delete_factor_list]
            ...
            bt.AZ_Delete_file(f'{self.tmp_mix_path}/{self.sector_name}', target_list=self.mix_factor_name_dict.values())
            print('delete all')

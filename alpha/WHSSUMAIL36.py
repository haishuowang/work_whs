...
class FactorTest(FactorTestBase, DiscreteClass, ContinueClass, TrainFunSet):
    ...
    def _get_mix_pnl_df(self, exe_str, cut_date, percent):
        def tmp_fun():
...
class ReturnData:
    def __init__(self, root_path, begin_date, end_date):
        ...
        return_df['IF01'] = self.load_index_data('000300')
        return_df['IC01'] = self.load_index_data('000905')
    ...
    def annual_return(self, pos_df, window=600, if_weight=1, ic_weight=0):
        ...
        pnl_df = (pos_df_cut * return_df_hedge).sum(1)
        pnl_df_l = (pos_df_cut[pos_df_cut > 0] * return_df_hedge).sum(1)
        pnl_df_s = (pos_df_cut[pos_df_cut < 0] * return_df_hedge).sum(1)

        annual_r = pnl_df.sum() / pos_df_cut.abs().sum(1).sum() * 250
        margin_l = pnl_df_l.sum() / pos_df_cut[pos_df_cut > 0].abs().sum(1).sum() * 250
        margin_s = pnl_df_s.sum() / pos_df_cut[pos_df_cut < 0].abs().sum(1).sum() * 250
...
def main_fun(str_1, exe_str):
    ...
    annual_r = pnl_df.iloc[-625:].sum() / pos_df.iloc[-625:].abs().sum(1).sum() * 250
    print('年华收益: ', annual_r)
...
if __name__ == '__main__':
    str_1 = 'market_top_300to800plus|20|True|0.1'
    exe_str = 'R_NetROA1_First|row_zscore_1.0@add_fun@PEG_PARENTNETPROFIT_5Y|col_zscore|60_-1.0@add_fun@' \
              'PEG_EBIT_3Y|col_zscore|60_-1.0@add_fun@R_OPCF_TTM_QSD4Y|pnd_vol|120_1.0@add_fun@' \
              'R_EMPLOYEEPAY_QTTM|row_zscore_1.0@add_fun@R_EMPLOYEEPAY_QTTM|pnd_vol|60_1.0@add_fun@' \
              'R_NetROA1_First|col_zscore|120_1.0@add_fun@R_NetAssets_s_POP_First|col_zscore|60_1.0@add_fun@' \
              'R_TotRev_TTM_Y3YGR|col_zscore|20_1.0@add_fun@R_NetWorkCapital_QTTM|col_zscore|5_-1.0@add_fun@' \
              'R_NETPROFIT_QTTM|pnd_vol|5_-1.0 '

...
class FactorTestSector(FactorTest):
    ...
    def load_daily_factor(self, file_name):
        ...
        load_path = f'/mnt/mfs/DAT_EQT/EM_Funda/daily/'
...
def config_test():
    # pass True True 0.021797 2.3913 2.826 3.6063 82.13 4.03 6.22 2.8528 2.56
    factor_str = 'turn_p60d_0.2|aadj_r_p60d_col_extre_0.2|BBANDS_200_1|aadj_r_p345d_continue_ud|evol_p10d' \
                 '|ADX_40_20_10|aadj_r_p120d_col_extre_0.2|wgt_return_p20d_0.2|evol_p30d' \
                 '|continue_ud_p200d|wgt_return_p60d_0.2'
    info_str = 'market_top_300plus_industry_20_25_30_35|20|False'
    #
    factor_str = 'price_p10d_hl|turn_p150d_0.18|moment_p510d|aadj_r_p345d_continue_ud|volume_moment_p530d' \
    ...
    # factor_str = 'price_p20d_hl|turn_p60d_0.2|volume_count_down_p10d|evol_p20d|continue_ud_p50d|evol_p10d' \
    #              '|MA_LINE_10_5|continue_ud_p100d|aadj_r_p345d_continue_ud|aadj_r_row_extre_0.2|MFI_40_70_30'
    # info_str = 'market_top_300to800plus|5|False'

    factor_name_list = factor_str.split('|')
    alpha_name = 'WHSZHIWEHN00'
    ...
    plot_send_result(pnl_df.iloc[-600:], bt.AZ_Sharpe_y(pnl_df.iloc[-600:]), alpha_name)
    corr_test_fun(pnl_df[-1250:], alpha_name)
    bt.commit_check(pd.DataFrame(pnl_df))
    margin_l, margin_s = bt.AZ_ls_margin(sum_pos_df_new.shift(2), main.return_choose, 1250)
    annual_r = bt.AZ_annual_return(sum_pos_df_new.shift(2), main.return_choose, 1250)
    # sum_pos_df_new.round(10).fillna(0).to_csv(f'/mnt/mfs/AAPOS/{alpha_name}.pos', sep='|', index_label='Date')
    print(margin_l, margin_s, annual_r)

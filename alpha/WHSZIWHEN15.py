...
def commit_check(pnl_df, mod='o'):
    ...
    print(matrix_corr_o)
    matrix_sp5 = pnl_df.iloc[-1250:].apply(bt.AZ_Sharpe_y)
    matrix_lv5 = pnl_df.iloc[-1250:].cumsum().apply(AZ_Leverage_ratio)

    matrix_sp2 = pnl_df.iloc[-500:].apply(bt.AZ_Sharpe_y)
    ...
    if mod == 'h':
        cond_matrix = pd.DataFrame([[0.5, 1.90, 1.66, 1.70, 1.70],
                                    [0.6, 2.00, 1.75, 1.75, 1.75],
                                    [0.7, 2.10, 1.80, 1.80, 1.80]])
    else:
        cond_matrix = pd.DataFrame([[0.5, 2.00, 1.75, 2.00, 2.00],
                                    [0.6, 2.10, 1.85, 2.10, 2.10],
                                    [0.7, 2.25, 1.95, 2.20, 2.20]])
    ...
    return result_df, info_df, matrix_corr_o
...
def commit_check_beta(pnl_df, mod='o'):
    ...
    all_pnl_df = pd.read_csv('/mnt/mfs/AATST/corr_tst_pnls', sep='|', index_col=0, parse_dates=True)
    all_pnl_df_c = pd.concat([all_pnl_df, pnl_df], axis=1)
    all_pnl_df_c_ma3 = bt.AZ_Rolling(all_pnl_df_c, 3).mean().iloc[-1250:]
    ...
    if mod == 'h':
        cond_matrix = pd.DataFrame([[0.47, 1.90, 1.66, 1.70, 1.70],
                                    [0.57, 2.00, 1.75, 1.75, 1.75],
                                    [0.67, 2.10, 1.80, 1.80, 1.80]])
    else:
        cond_matrix = pd.DataFrame([[0.47, 2.00, 1.75, 2.00, 2.00],
                                    [0.57, 2.10, 1.85, 2.10, 2.10],
                                    [0.67, 2.25, 1.95, 2.20, 2.20]])
...
def config_test():
    ...
    _, _, a = commit_check(pd.DataFrame(pnl_df))
    _, _, b = commit_check_beta(pd.DataFrame(pnl_df))

...
import work_whs.AZ_2018_Q2.factor_script.main_file.main_file_single_test_remy as mfstr
    ...
    for sector_name in sector_name_list:
        ...
        for (hold_time, if_only_long), part_df in data.groupby(['hold_time', 'if_only_long']):
            ...
            if if_only_long == 'True':
                ...
                # pnl_table = pnl_table[[x for x in pnl_table.columns if x.startswith('REM') and len(x.split('|')[0]) > 10]]
                # remy pnl
                    ...
                    if target_sharpe > 2:
                        ...
                        if sp > 2:
                            ...
                            result_df, info_df = bt.commit_check(pd.DataFrame(pnl_df,
                                                                              columns=['|'.join([x.split('|')[0] for x
                                                                                                 in portfolio_index])]))
                            print(result_df)
                            # if (result_df.values == 1).all():
                            plot_send_result(pnl_df, sp, '[RESULT]', info_df.to_html() +
...
def part_main_fun(sector_name, file_name):
    ...
    if abs(sp) > 1.5:
        print(file_name, sp)
        plot_send_result(pnl_df, sp, f'[RESULT]{file_name}', file_name)
    # elif abs(sp) > 1.2:
    #     print(file_name, sp)
    ...
    for sector_name in sector_name_list:
        ...
        for (hold_time, if_only_long), part_df in data.groupby(['hold_time', 'if_only_long']):
            ...
            if len(x) != 0:
                ...
                plot_send_result(all_pnl_df, bt.AZ_Sharpe_y(all_pnl_df), '[RESULT]',

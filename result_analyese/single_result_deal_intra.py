...
import work_whs.main_file.main_file_single_test_intra as mfstr
...
def get_file_name(sector_name):
    tmp_file_list = os.listdir(f'/media/hdd2/dat_whs/data/single_factor_pnl_intra/{sector_name}')
...
def get_pnl_table(part_df, sector_name):
    ...
    for file_name in file_name_df:
        ...
        pnl_df = pd.read_csv(f'/media/hdd2/dat_whs/data/single_factor_pnl_intra/{sector_name}/{file_name}',
...
intra_factor_dict = dict({
    'intra_oc_15min_volume_div.csv': 'load_intra_factor',
    'intra_last_15min_volume_pct.csv': 'load_intra_factor',
    'intra_hl_pct.csv': 'load_intra_factor',
    'intra_today_sharpe.csv': 'load_intra_factor',
    'intra_last_15_min_ud.csv': 'load_intra_factor',
    'intra_r_vol_last_15min.csv': 'load_intra_factor',
    'intra_r_vol.csv': 'load_intra_factor',
    'intra_p_vol_last_15min.csv': 'load_intra_factor',
    'intra_p_vol.csv': 'load_intra_factor',
    'intra_money_flow2.csv': 'load_intra_factor',
    'intra_money_flow1.csv': 'load_intra_factor',
    'intra_dn_15_bar_div_daily.csv': 'load_intra_factor',
    'intra_up_15_bar_div_daily.csv': 'load_intra_factor',
    'intra_up_15_bar_div_dn_15_bar.csv': 'load_intra_factor',
    'intra_dn_div_daily.csv': 'load_intra_factor',
    'intra_up_div_daily.csv': 'load_intra_factor',
    'intra_up_div_dn.csv': 'load_intra_factor',
    'intra_dn_vwap.csv': 'load_intra_factor',
    'intra_up_vwap.csv': 'load_intra_factor',
    'intra_dn_15_bar_vwap.csv': 'load_intra_factor',
    'intra_up_15_bar_vwap.csv': 'load_intra_factor',
    'intra_daily_vwap.csv': 'load_intra_factor',
    'intra_dn_15_bar_vol.csv': 'load_intra_factor',
    'intra_up_15_bar_vol.csv': 'load_intra_factor',
    'intra_dn_vol.csv': 'load_intra_factor',
    'intra_up_vol.csv': 'load_intra_factor',
})

my_factor_dict.update(my_factor_dict_2)
my_factor_dict.update(jerry_factor_dict)
my_factor_dict.update(intra_factor_dict)
sector_name_list = [
    'market_top_300plus',
...
def deal_fun(sector_name, hold_time, if_only_long, name_list, buy_sell_way_list):
    ...
    main = mfstr.FactorTestSector(root_path, if_save, if_new_program, begin_date, cut_date, end_date, [], sector_name,
    ...
    for sector_name in sector_name_list:
        ...
        for (hold_time, if_only_long), part_df in data.groupby(['hold_time', 'if_only_long']):
            ...
            for file_name in sharpe_up.index:
                ...
                modify_index = corr_series.loc[corr_series.abs().sort_values().index].index[:4]
                ...
                if target_sharpe > 2:
                    ...
                    plot_send_result(pnl_df, sp, '[RESULT]', '|'.join([x.split('|')[0] for x in portfolio_index]) + '$'
...
def part_main_fun(sector_name, file_name):
    pnl_df = pd.read_csv(f'/media/hdd2/dat_whs/data/single_factor_pnl_intra/{sector_name}/{file_name}',
                         index_col=0, parse_dates=True)
    pnl_df.columns = [file_name]
    sp = bt.AZ_Sharpe_y(pnl_df).values[0]

    # if abs(sp) > 1.5:
    #     print(file_name, sp)
    #     plot_send_result(pnl_df, sp, '[RESULT]1.5', file_name)
    # elif abs(sp) > 1.2:
    #     print(file_name, sp)
    #     plot_send_result(pnl_df, sp, '[RESULT]1.2', file_name)
    # elif abs(sp) > 1:
    #     print(file_name, sp)
    #     plot_send_result(pnl_df, sp, '[RESULT]1', file_name)
    if abs(sp) >= 1.5:
        if sp > 0:
            return pnl_df
        else:
...
def main_fun_1():
    for sector_name in sector_name_list:
        ...
        for (hold_time, if_only_long), part_df in data.groupby(['hold_time', 'if_only_long']):
            ...
            for file_name in file_name_df:
                # print(file_name)
                # part_main_fun(sector_name, file_name)
                result_list.append(pool.apply_async(part_main_fun, (sector_name, file_name)))
            pool.close()
            pool.join()

            x = [res.get() for res in result_list if res.get() is not None]
            print(len(x))
            if len(x) != 0:
                print('send_email')
                all_pnl_df = pd.concat(x, axis=1).mean(1)
                plot_send_result(all_pnl_df, bt.AZ_Sharpe_y(all_pnl_df), '[RESULT]1',
                                 '|'.join([sector_name, str(hold_time), str(if_only_long)]))

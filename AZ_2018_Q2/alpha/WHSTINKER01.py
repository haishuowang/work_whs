...
def config_test():
    config_set = pd.read_pickle(f'/media/hdd1/DAT_PreCalc/PreCalc_whs/CRTTINKER01.pkl')
    config_data = config_set['factor_info']
    sector_name = config_set['sector_name']
    alpha_name = 'WHSTINKER01'
    cut_date = None
    ...
    # if os.path.exists('/mnt/mfs/AAPOS/WHSMEDUSA01.pos'):
    #     sum_pos_df_old = bt.AZ_Load_csv('/mnt/mfs/AAPOS/WHSMEDUSA01.pos')
    #     sum_pos_df = sum_pos_df_old.combine_first(sum_pos_df_new)
    #     # pnl_df = (sum_pos_df.shift(2) * main.return_choose).sum(axis=1)
    #     # plot_send_result(pnl_df.iloc[-250:], bt.AZ_Sharpe_y(pnl_df.iloc[-250:]), 'WHSMEDUSA01')
    #     sum_pos_df.round(10).to_csv('/mnt/mfs/AAPOS/WHSMEDUSA01.pos', sep='|', index_label='Date')
    #     return sum_pos_df
    # else:

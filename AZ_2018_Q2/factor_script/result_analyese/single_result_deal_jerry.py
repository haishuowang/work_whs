...
import work_whs.AZ_2018_Q2.factor_script.main_file.main_file_single_test_jerry as mfstr
...
def get_pnl_table(part_df, sector_name):
    ...
    for file_name in file_name_df:
        # print(file_name)
        if file_name.startswith('vr') or file_name.startswith('M1') \
                or file_name.startswith('LIQ') or file_name.startswith('M0'):
...
def main_fun():
    for sector_name in sector_name_list:
        ...
        for (hold_time, if_only_long), part_df in data.groupby(['hold_time', 'if_only_long']):
            ...
            jerry_list = list(jerry_factor_dict.keys())
            ...
            remy_list = [x for x in pnl_table.columns if x.split('|')[0] in jerry_list]

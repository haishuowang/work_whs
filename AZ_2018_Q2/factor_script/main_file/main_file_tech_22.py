    ...
    def part_test_index(self, key, name_1, log_save_file, result_save_file, total_para_num, my_factor_dict):
        try:
            ...
            *result_list, pnl_df = filter_all(self.cut_date, daily_pos, self.return_choose,
            ...
            if abs(bt.AZ_Sharpe_y(pnl_df)) > 0.8:
                plot_send_result(pnl_df, bt.AZ_Sharpe_y(pnl_df),
                                 f'[BKTRESULT]{self.sector_name}|{self.if_only_long}|{self.hold_time}|{name_1}'
                                 f'|{bt.AZ_Sharpe_y(pnl_df)}', text='|'.join([str(x) for x in result_list]))
            ...
            print('{}%, {}, cost {} seconds, load_cost {} seconds'
                  .format(round(key / total_para_num * 100, 4), key, name_1, round(end_time - start_time, 2),
                          load_delta))
        except Exception as error:
            send_email.send_email(error, ['whs@yingpei.com'], [], f'[BKTERROR]{name_1}|{self.sector_name}'
                                                                  f'|{self.hold_time}|{self.if_only_long}')
            print(error)
